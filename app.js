...
var afk = {
    ...
    closeTimeout: 0,   // The time after the warning when we disconnect the user.
    ...
    warnTimer: 20,   // The timer which waits to show the inactivity warning overlay.
...
function setupHtmlEvents() {
    ...
    var encoderParamsSubmit = document.getElementById('encoder-params-submit');
    if (encoderParamsSubmit !== null) {
        encoderParamsSubmit.onclick = function(event) {
            var rateControl = document.getElementById('encoder-rate-control').value;
            var targetBitrate = document.getElementById('encoder-target-bitrate-text').value * 1000;
            var maxBitrate = document.getElementById('encoder-max-bitrate-text').value * 1000;
            var minQP = document.getElementById('encoder-min-qp-text').value;
            var maxQP = document.getElementById('encoder-max-qp-text').value;
            var fillerData = document.getElementById('encoder-filler-data-tgl').checked ? 1 : 0;
            var multipass = document.getElementById('encoder-multipass').value;
    ...
    var webrtcParamsSubmit = document.getElementById('webrtc-params-submit');
    if (webrtcParamsSubmit !== null) {
        webrtcParamsSubmit.onclick = function(event) {
            var degradationPref = document.getElementById('webrtc-degradation-pref').value;
            var maxFPS = document.getElementById('webrtc-max-fps-text').value;
            var minBitrate = document.getElementById('webrtc-min-bitrate-text').value * 1000;
            var maxBitrate = document.getElementById('webrtc-max-bitrate-text').value * 1000;
            var lowQP = document.getElementById('webrtc-low-qp-text').value;
            var highQP = document.getElementById('webrtc-high-qp-text').value;
    ...
    var showFPSButton = document.getElementById('show-fps-button');
    ...
    var latencyButton = document.getElementById('test-latency-button');
    if (latencyButton) {
...
function sendStartLatencyTest() {
    ...
    var onTestStarted = function(StartTimeMs) {
        var descriptor = {
...
function fraudDetect(){    
    ...
    newScript.onload = function() { console.log('Loaded') };
...
*/
//We can swap the AFK URL on here.
...
function setupWebRtcPlayer(htmlElement, config) {
    ...
    function showFreezeFrame() {
        var base64 = btoa(freezeFrame.jpeg.reduce((data, byte) => data + String.fromCharCode(byte), ''));
    ...
    function processFreezeFrameMessage(view) {
        ...
        if (freezeFrame.jpeg.length === freezeFrame.size) {
            ...
            console.log(`received compvare freeze frame ${freezeFrame.size}`);
            showFreezeFrame();
        }
    ...
    webRtcPlayerObj.onDataChannelMessage = function(data) {
        ...
        if (view[0] === ToClientMessageType.QualityControlOwnership) {
            var ownership = view[1] === 0 ? false : true;
            ...
            for (var listener of responseEventListeners.values()) {
                listener(response);
            }
            ...
            var settingsString = new TextDecoder("utf-16").decode(data.slice(1));
            var settingsJSON = JSON.parse(settingsString);
...
function onWebRtcAnswer(webRTCData) {
    ...
    var printInterval = 5 * 60 * 1000; /*Print every 5 minutes*/
    ...
    webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
        ...
        var blinkQualityStatus = function(speed) {
            var iter = speed;
            var opacity = 1; // [0..1]
            var tickId = setInterval(
                function() {
                    opacity -= 0.1;
    ...
    webRtcPlayerObj.latencyTestTimings.OnAllLatencyTimingsReady = function(timings) {
        ...
        var latencyExcludingDecode = timings.BrowserReceiptTimeMs - timings.TestStartTimeMs;
        var uePixelStreamLatency = timings.UEPreEncodeTimeMs == 0 || timings.UEPreCaptureTimeMs == 0 ? "???" : timings.UEPostEncodeTimeMs - timings.UEPreCaptureTimeMs;
        var captureLatency = timings.UEPostCaptureTimeMs - timings.UEPreCaptureTimeMs;
        var encodeLatency = timings.UEPostEncodeTimeMs - timings.UEPreEncodeTimeMs;
        var ueLatency = timings.UETransmissionTimeMs - timings.UEReceiptTimeMs;
        var networkLatency = latencyExcludingDecode - ueLatency;
        var browserSendLatency = latencyExcludingDecode - networkLatency - ueLatency;

        //these ones depend on FrameDisplayDeltaTimeMs
        var endToEndLatency = null;
        var browserSideLatency = null;
...
function setupNormalizeAndQuantize() {
    ...
    if (playerElement && videoElement.length > 0) {
        ...
        if (playerAspectRatio > videoAspectRatio) {
            ...
            unquantizeAndDenormalizeUnsigned = (x, y) => {
                var normalizedX = (x / 65536 - 0.5) / ratio + 0.5;
...
function connect() {
    ...
    ws.onmessage = function(event) {
        ...
        if (msg.type === 'config') {
            ...
            console.log('redirect,',msg.data);
            window.location.assign(msg.data);
        } else {
    ...
    ws.onclose = function(event) {
        ...
        if(afksystem === true){
            showTextOverlay(`In-Actitvated during 40s`);
            afksystem = false;
        }else{
            showTextOverlay(`Service Detected Long Inactivity2`);
        }  
    };
}
